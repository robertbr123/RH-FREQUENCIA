    ...
    try {
      ...
      const birthdaysThisMonth = employees
        .filter((emp: any) => {
          ...
          // Extrair mês e dia diretamente da string YYYY-MM-DD para evitar problemas de timezone
          const birthDateStr = String(emp.birth_date).split('T')[0] // Pega só a parte da data (YYYY-MM-DD)
          console.log(`🔍 Processando ${emp.name}: ${birthDateStr}`)
          
          const [year, month, day] = birthDateStr.split('-').map(Number)
          const birthMonth = month // Já vem no formato 1-12
          const birthDay = day
          
          console.log(`   📆 Data extraída: ${birthDay}/${birthMonth}/${year}`)
        ...
        .sort((a: any, b: any) => {
          // Ordenar por dia do mês - extrair diretamente da string
          const dayA = parseInt(String(a.birth_date).split('T')[0].split('-')[2], 10)
          const dayB = parseInt(String(b.birth_date).split('T')[0].split('-')[2], 10)
          return dayA - dayB
        })
