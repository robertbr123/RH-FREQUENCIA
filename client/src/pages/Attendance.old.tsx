...
export default function Attendance() {
  ...
  const exportToCSV = () => {
    ...
    const rows = filteredRecords.map(r => {
      // Formatar horários sem problemas de timezone
      const formatTime = (dateStr: string) => {
        if (!dateStr) return '-'
        // Se é apenas horário (HH:MM:SS), adicionar data de hoje
        if (dateStr.match(/^\d{2}:\d{2}:\d{2}$/)) {
          return dateStr.substring(0, 5) // Retorna apenas HH:MM
        }
        // Se é data completa, extrair apenas o horário ou usar date-fns com timezone
        const date = new Date(dateStr + 'Z') // Adiciona Z para forçar UTC e depois ajustar
            ...
            <table className="w-full">
              ...
              <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                {filteredRecords.map((record) => {
                  // Função para formatar horário sem problemas de timezone
                  const formatTime = (dateStr: string) => {
                    if (!dateStr) return '-'
                    
                    // Se é apenas horário (HH:MM:SS ou HH:MM), retornar direto
                    if (dateStr.match(/^\d{2}:\d{2}(:\d{2})?$/)) {
                      return dateStr.substring(0, 5) // Retorna apenas HH:MM
                    }
                    
                    // Se é timestamp completo com data
                    // Extrair a parte da data e hora sem conversão de timezone
                    if (dateStr.includes('T')) {
                      const [datePart, timePart] = dateStr.split('T')
                      const [year, month, day] = datePart.split('-')
                      const time = timePart.split('.')[0].substring(0, 5) // HH:MM
                      return `${day}/${month}/${year} ${time}`
                    }
                    
                    // Fallback: tentar parsear como data normal
                    try {
